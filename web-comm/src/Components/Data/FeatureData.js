import { Rocket, Profile, Security, Earth } from "../../assets/icons";

export const FEATURES_DATA = [
  {
    id: 1,
    title: "Video Calling",
    message:
      "Channels brings order and clarity to work - you can create them for every project, topic or team. When there's a channel for everything, you can focus on the conversations and work that matters most to you.",
  },
  {
    id: 2,
    title: "Video Calling",
    message:
      "Channels brings order and clarity to work - you can create them for every project, topic or team. When there's a channel for everything, you can focus on the conversations and work that matters most to you.",
  },
  {
    id: 3,
    title: "Messaging",
    message:
      "Channels brings order and clarity to work - you can create them for every project, topic or team. When there's a channel for everything, you can focus on the conversations and work that matters most to you.",
  },
  {
    id: 4,
    title: "Conferencing Calling",
    message:
      "Channels brings order and clarity to work - you can create them for every project, topic or team. When there's a channel for everything, you can focus on the conversations and work that matters most to you.",
  },
  {
    id: 5,
    title: "File Sharing",
    message:
      "Channels brings order and clarity to work - you can create them for every project, topic or team. When there's a channel for everything, you can focus on the conversations and work that matters most to you.",
  },
  {
    id: 6,
    title: "Cloud Storage",
    message:
      "Channels brings order and clarity to work - you can create them for every project, topic or team. When there's a channel for everything, you can focus on the conversations and work that matters most to you.",
  },
];

export const FEATURES_SERVICES = [
  {
    id: 1,
    title: "Communicate with, and integrate Voice Calls on the Go.",
    message:
      "Stay productive by seamlessly integrating voice calls into your mobile workflow, ensuring convenient communication and effective collaboration on the move.",
    icon: Rocket,
  },
  {
    id: 2,
    title: "Organize Teams & Departments.",
    message:
      "Efficiently organize teams and departments with our solution, streamlining collaboration and enhancing productivity in your organization.",
    icon: Profile,
  },
  {
    id: 3,
    title: "Secured messages for the employess anywhere.",
    message:
      "Ensure secure communication with our solution, providing platform for confidential and protected interactions in your business.",
    icon: Security,
  },
  {
    id: 4,
    title: "Share files and collaborate anywhere.",
    message:
      "Connect and collaborate effortlessly from anywhere in the world with our solution, breaking down geographical barriers for seamless communication.",
    icon: Earth,
  },
];

export const FAQ = [
  {
    id: 1,
    title: "How do I connect with an external company?",
    message:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Assumenda accusantium reprehenderit dolore expedita distinctio facilis quam id sint, laborum nostrum, non facere consequatur iure, optio quae iusto impedit repudiandae ratione dicta! Deserunt delectus reprehenderit quis obcaecati minus recusandae aliquam, molestiae, expedita rerum illum iusto ipsum quae vel quos repudiandae nemo.",
  },
  {
    id: 2,
    title: "What are some best practices for channels?",
    message:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Assumenda accusantium reprehenderit dolore expedita distinctio facilis quam id sint, laborum nostrum, non facere consequatur iure, optio quae iusto impedit repudiandae ratione dicta! Deserunt delectus reprehenderit quis obcaecati minus recusandae aliquam, molestiae, expedita rerum illum iusto ipsum quae vel quos repudiandae nemo.",
  },
  {
    id: 3,
    title: "Is there a way to convert an existing channel to a shared channel?",
    message:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Assumenda accusantium reprehenderit dolore expedita distinctio facilis quam id sint, laborum nostrum, non facere consequatur iure, optio quae iusto impedit repudiandae ratione dicta! Deserunt delectus reprehenderit quis obcaecati minus recusandae aliquam, molestiae, expedita rerum illum iusto ipsum quae vel quos repudiandae nemo.",
  },
  {
    id: 4,
    title: "What's the difference between group chats and Slack channels?",
    message:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Assumenda accusantium reprehenderit dolore expedita distinctio facilis quam id sint, laborum nostrum, non facere consequatur iure, optio quae iusto impedit repudiandae ratione dicta! Deserunt delectus reprehenderit quis obcaecati minus recusandae aliquam, molestiae, expedita rerum illum iusto ipsum quae vel quos repudiandae nemo.",
  },
];
